...
export default function ListaCompras({ onBack }) {
  ...
  useEffect(() => {
    saveListaCompras({ marcados, propios });
  }, [marcados, propios]);
  ...
  const totalItems = useMemo(
    () => listaRecomendada.reduce((acc, c) => acc + c.items.length, 0) + propios.length,
    [propios]
  );
      ...
      <div className="bg-white rounded-[20px] border border-[var(--border-soft)] p-5 shadow-sm mb-6">
        <div className="flex items-center justify-between mb-2">
          <p className="text-sm font-medium text-ink">
            {totalMarcados} de {totalItems} comprado
          </p>
